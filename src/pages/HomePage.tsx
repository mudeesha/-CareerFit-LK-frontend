  ...
  useEffect(() => {
    async function loadHomeData() {
      try {
        ...
        const [jobsResponse, categoriesResponse, companiesResponse] =
          await Promise.all([getJobs(), getCategories(), getCompanies()]);

        setJobs(jobsResponse.items.slice(0, 6));
        setCategories(categoriesResponse.items.slice(0, 8));
        setCompanies(companiesResponse.items.slice(0, 6));
      } finally {
        setIsLoading(false);
      }
    }
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {[1, 2, 3, 4, 5, 6].map((item) => (
                <div
                  key={item}
                  className="h-52 bg-[#F3F4F6] border border-gray-200 rounded-[20px] animate-pulse"
                />
              ))}
            </div>
          ) : jobs.length > 0 ? (
          ...
          {isLoading ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {[1, 2, 3, 4].map((item) => (
                <div
                  key={item}
                  className="h-40 bg-white border border-gray-200 rounded-[20px] animate-pulse"
                />
              ))}
            </div>
          ) : categories.length > 0 ? (
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-purple-50 p-4 rounded-xl border border-purple-100">
                    ...
                    <p className="text-sm text-gray-900">
                      Add a TypeScript project to your CV to increase your match
                      score.
                    </p>
                  </div>
                </div>
